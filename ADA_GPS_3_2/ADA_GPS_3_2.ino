...
if (StatDatiFile = SD.open(StatDatiPath.c_str(), O_RDWR)) {
  ...
  // Versione originale senza spazi dopo le virgole
  // sprintf(StatDatiBuffer, "%s,%s,%.2f,%.2f,%.2f,%.2f,%.2f",
  
  // Versione modificata con spazi dopo le virgole (per coerenza con MaxDati.txt)
  sprintf(StatDatiBuffer, "%s, %s, %.2f, %.2f, %.2f, %.2f, %.2f",
...
void calibrateSensors() {
   float FieldPressure = baro.getPressure();
  float FieldTemperature = baro.getTemperature();
    baro.setSeaPressure(FieldPressure);
  float initialAltitude = baro.getAltitude();
   if (initialAltitude != 0.0) {
    float correctedSeaPressure = FieldPressure * pow(1 - (initialAltitude / 44330.0), 5.255);
...
void calculateVario(float currentAltitude, float currentTime, float currentSpeed) {
  ...
  if (previousTime != 0 && currentTime > previousTime + 0.8) {  // Avoid division by zero and ensure meaningful time interval. Ptrev TYime is for first run only
    ...
    const float alpha = 0.85;  // Adjust this value to control smoothing
