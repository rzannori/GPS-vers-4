...
void calculateVario() {
  ...
  if (flightData.vario > VARIO_THRESHOLD) {  // Inizia a misurare solo quando il vario è consistentemente positivo
    if (!thermalData.isPositivePeriod) {
      // Verifica se il vario è stato consistentemente positivo
    ...
    if (millis() - lastStableTime >= STABLE_DURATION) {
      ...
      thermalData.gainedAltitude = thermalData.endAltitude - thermalData.startAltitude;
      thermalData.totGainedAltitude += thermalData.gainedAltitude;
